...
- `packages/sources` — normalized `SourcedPackage` type, one subfolder per
  ...
  "Source cache" below. Flathub, Snapcraft, AppImage, AUR, and Debian's
  `fetch.ts` are all implemented (e.g. `pnpm --filter @tuxery/sources
refresh flathub`) — see [`docs/sources.md`](docs/sources.md) for status
  per source.
