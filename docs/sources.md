...
| 5h  | Other native          | —                    | Native   | —       | ✅          | Roadmap     | [13]  |
| 6   | Nixpkgs               | —                    | Native   | 131,101 | ✅          | Implemented | [14]  |
...
13. **Other native** (Void, Gentoo, Solus, Clear Linux, Slackware) — each
    has its own repodata format; Void is the same "repodata file(s), per
    repo/arch" shape as the sources above; Gentoo/Solus/Slackware/Clear
    Linux are niche enough to be low priority. Roadmap — Void worth
    prioritizing over the rest when picked up. NixOS/nixpkgs split out to
    its own row (6), openSUSE to row 5f, Alpine to row 5g — see
    above/below.
14. **Nixpkgs** — `channels.nixos.org/nixos-unstable/packages.json.br`, a
