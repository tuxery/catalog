...
- **Scheduled refresh**: `.github/workflows/refresh-sources.yml` runs every `fetch.ts` weekly (Monday 4am UTC, or on demand via `workflow_dispatch` for a single source), opening a PR with any cache diff rather than pushing straight to main (branch-protected, PR + rebase only) — one source failing doesn't block the others, it's just left at its last successful cache state.
- **GUI vs CLI classification**: `SourcedPackage.hasDesktopFile` (Fedora/openSUSE only, parsed from `<rpm:provides>`'s synthetic `application(*.desktop)` entry — RPM tooling generates this for any package shipping a `.desktop` file) feeds `@tuxery/curator`'s enrich stage, which sets `CatalogApp.kind = "gui"` when any member package has it. Deliberately one-directional — absence never sets `"cli"`, since most real GUI apps don't happen to trigger this specific convention (~3% coverage on real Fedora/openSUSE data, though 100% precise where present). See the "GUI vs CLI classification" card for the broader plan (a weaker Debian/Ubuntu Section-based heuristic, Arch/AUR signal) — not yet implemented, this is the first slice.
