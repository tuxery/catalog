...
const CategoryRuleEntrySchema = z.object({
  ...
  category: AppCategoryLabelSchema.describe(
    "One of config/categories-apps.json's own display labels (not a freedesktop key) — this is a last-resort signal for apps with no upstream category at all, so it writes the label directly. Never a categories-games.json genre — games have their own equivalent file, config/game-category-rules.json (see game-category-rules.ts), since the two taxonomies are disjoint closed enums.",
  ),
  reason: z
    .string()
    .describe(
